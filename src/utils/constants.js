...
const GoldenReels = {
    ...
    // comfortably inside Discord's 15-minute follow-up token window for editReply, so no
    // interaction-expiry risk, just a longer session if someone runs it all the way.
    MAX_SPINS: 100,
    SPIN_DELAY_MS: 2000
}
...
// a guild (see guildCompanionFactory.js's donate/withdraw functions and systems/guilds.md's
// "Guild Companion (Cinderroot) Rework" section). `guild.guildCompanion` itself keeps its
// original two-field shape (`{ id, acquiredAt, acquiredRaidTier }`, no `equipped` flag) — a
// same-day revision briefly added then removed an equip/unequip toggle, see
