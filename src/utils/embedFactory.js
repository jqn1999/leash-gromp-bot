...
class EmbedFactory {
    ...
    // powerBreakdown: raidFactory.getEffectiveRaidPowerBreakdown's { teamPower,
    // headcountBonus, effectivePower } — shown in the description so the Total Multiplier
    // in the title isn't just an opaque number: top raider counted fully, each
    // next-strongest counted at RAID_TEAM_DECAY of the rank above them, then bumped by a
    // headcount bonus for roster size (see raidFactory.getEffectiveRaidPower's own
    // comment for the full rank-weighted shape). Optional and defaults to null so any
    ...
    createRaidEmbed(guildName, raidList, raidCount, totalRaidReward, splitRaidReward, mob, successChance,
        ...
        if (Array.isArray(splitRaidReward)) {
            // Contribution-based split (guild.raidSplitMode: 'share') — each entry
            // already carries its own raidSplitAmount from
            // raidFactory.handlePotatoSplitByShare, computed against whatever remainder
            // didn't fit in the guild bank. Shown per-member since an uneven split is the
            // entire point of this mode; recomputing raidShare * totalRaidReward here
            // instead would ignore however much the bank already absorbed.
            fields.push({
                name: `Split Potatoes ${gainOrLoss} (Contribution-Based):`,
                value: splitRaidReward.map(m => `${m.username}: ${m.raidSplitAmount.toLocaleString()} potatoes`).join('\n'),
                inline: false,
            })
        } else if (splitRaidReward) {
            fields.push({
