...
describe('Bounty.TIERS ladder shape', () => {
    ...
    // penalty:reward ratio itself climbs continuously from 1.0x at B1 to 2.0x at B12
    ...
    // A same-day 2026-09-19 flirtation with flattening penalty to one shared amount across
    // every tier (misreading "stay at the base amount" as "identical for every tier" rather
    // than its real meaning, "doesn't scale with the mercenary's own Rank") was reverted the
    // same day — this climbing-ratio shape is unchanged from its original 2026-09-08 design.
...
// from bounties and rob-npc is decreased by half, rounding down, minimum of 1." Threshold
// raised 20 -> 50 on 2026-09-13, direct instruction ("make the notoriety halving start at
// 50 instead of 20"). Keyed off Rival.NOTORIETY_GAIN_HALVING_THRESHOLD, a SEPARATE constant
// from Rival.CONFRONTATION_THRESHOLD (still 20, the /confront-rival unlock gate).
describe('getNotorietyGain', () => {
    test('at or below the threshold, gain is untouched', () => {
        ...
        expect(mercenaryFactory.getNotorietyGain(Rival.NOTORIETY_GAIN_HALVING_THRESHOLD, 4)).toBe(4);
    });

    test('above the threshold, gain is halved and rounded down', () => {
        expect(mercenaryFactory.getNotorietyGain(Rival.NOTORIETY_GAIN_HALVING_THRESHOLD + 1, 4)).toBe(2);
        expect(mercenaryFactory.getNotorietyGain(Rival.NOTORIETY_GAIN_HALVING_THRESHOLD + 1, 3)).toBe(1); // floor(1.5) = 1
    ...
    test('above the threshold, a halved gain is floored at a minimum of 1, never 0', () => {
        expect(mercenaryFactory.getNotorietyGain(Rival.NOTORIETY_GAIN_HALVING_THRESHOLD + 1, 1)).toBe(1);
    });

    test('above CONFRONTATION_THRESHOLD but at/below the (higher) halving threshold, gain is still full', () => {
        expect(mercenaryFactory.getNotorietyGain(Rival.CONFRONTATION_THRESHOLD + 1, 4)).toBe(4);
    });
});
