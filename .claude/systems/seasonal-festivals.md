...
- **`/admin-start-festival` shipped as its own standalone top-level command in this build's isolated
  worktree** (`src/commands/moderation/admin.js` didn't exist there yet — a separate, unrelated
  session was consolidating several `admin-*` commands into one at the same time), then folded into
  `/admin start-festival` as a new Subcommand during the merge back into the session branch, per the
  build brief's own contingency instruction — see `roadmap.md`'s merge entry.
