...
| `Potions` | Trading Post's static 3-potion catalog (`CATALOG`) — one entry per effect type (`workMulti`/`workTimer`/`passiveAmount`), each with `id`/`name`/`value`/`durationSeconds`/`pricePotatoes`, no stock tracking | [systems/trading-post.md](../systems/trading-post.md) |
