...
| `adminStartFestival.js` | `/admin-start-festival festival:<harvest_festival\|frost_fair\|spring_planting> duration_days:<1-14>` | `devOnly` + Administrator — starts a Seasonal Festival window, persisted (survives a restart) via `active_festival` — see [systems/seasonal-festivals.md](../systems/seasonal-festivals.md). Standalone command for now; fold into a consolidated `/admin` command if/when that consolidation lands |
