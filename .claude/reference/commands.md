...
| `adminResetTower.js` | `/admin-reset-tower player:<mention>` | `devOnly` + Administrator — restores a target player's `canEnterTower` to `true`, unsticking them from a crashed/stuck Tower run without waiting for the next day's 4am UTC reset — see [systems/tower.md](../systems/tower.md#admin-reset-tower) |
